...
function linuxDeviceKeyProviderSource({ childProcessVar, cryptoVar, fsVar, pathVar }) {
  return [
    ...
    `let codexLinuxRemoteControlSharedConfigDirectory=${pathVar}.join(codexLinuxRemoteControlConfigRoot,\`hydex-desktop\`);${fsVar}.mkdirSync(codexLinuxRemoteControlSharedConfigDirectory,{recursive:!0});`,
...
function applyLinuxRemoteMobileConversationHydrationPatch(source) {
  ...
  if (!patched.includes(REMOTE_MOBILE_HYDRATION_MARKER)) {
    ...
    if (
      ...
      const currentDispatcher = handlerContract.kind === "current";
...
function applyLinuxRemoteMobileReasoningSummaryPatch(source) {
  ...
  const currentCallerPattern = new RegExp(
    `(?<prefix>${escapeRegExp(helperName)}\\((?<manager>[A-Za-z_$][\\w$]*),` +
      ...
      `(?=canUseProjectlessWorkspace:!(?<classifier>[A-Za-z_$][\\w$]*)\\(\\k<manager>\\.getHostId\\(\\)\\)(?:\\|\\|\\k<conversation>\\.mode===\`durable\`\\|\\|!1)?,[\\s\\S]{0,1000}?` +
